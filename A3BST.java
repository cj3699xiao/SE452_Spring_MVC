...
public class A3BST<K extends Comparable<? super K>, V extends Comparable<? super V>> {
	private Node<K,V> root;             // root of BST

	private static class Node<K extends Comparable<? super K>,V extends Comparable<? super V>> {
	...
	public int countTwins() {return countTwins(root);}
	private int countTwins(Node<K, V> node) {
		if (node == null) return 0;
		Node<K, V> r = node.right;
		Node<K, V> l = node.left;
		int n = 0;
		if (r != null && l != null) n+=1;
		return n + countTwins(r) + countTwins(l);
	}
	
	public int lessThanValueCount(V value) {
		return lessThanValueCount(root, value);
	}
	private int lessThanValueCount(Node<K, V> node, V value) {
		if (node == null) return 0;
		int n = 0;
		if (node.val.compareTo(value) < 0) n += 1;
		return n + lessThanValueCount(node.right, value) + lessThanValueCount(node.left, value);
	}
	/* ***************************************************************************
	 *  Visualization
	 *****************************************************************************/
